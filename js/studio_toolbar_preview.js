(function () {
    ...
    function createDefaultState() {
        return {
            ...
            topbar: { bgColor: '#ffffff', textColor: '#1c1c1e', iconColor: '#1c1c1e', bgOpacity: 75, blur: 20, height: 44, dividerColor: '#000000', dividerOpacity: 8, dividerWidth: 1, backgroundImage: '', backIconUrl: '', fireIconUrl: '', settingsIconUrl: '', shadowEnabled: true, shadowColor: '#000000', shadowOpacity: 3, shadowBlur: 8, avatarVisible: false, avatarPosition: 'left', statusVisible: false, statusText: '', titleFontSize: 17, titleOffsetX: 0, titleOffsetY: 0, statusFontSize: 11, statusOffsetX: 0, statusOffsetY: 0, backButtonSize: 28, backButtonOffsetX: 0, backButtonOffsetY: 0, avatarButtonSize: 34, avatarButtonOffsetX: 0, avatarButtonOffsetY: 0, settingsButtonSize: 28, settingsButtonOffsetX: 0, settingsButtonOffsetY: 0 },
            bottombar: { barBgColor: '#ffffff', inputBgColor: '#ffffff', iconColor: '#8f8f96', barOpacity: 75, blur: 20, height: 83, inputRadius: 18, inputHeight: 36, inputBorderColor: '#d3d3da', inputBorderWidth: 1.5, dividerColor: '#000000', dividerOpacity: 8, dividerWidth: 1, backgroundImage: '', menuIconUrl: '', stickerIconUrl: '', replyIconUrl: '', menuButtonSize: 34, menuButtonOffsetX: 0, menuButtonOffsetY: 0, stickerButtonSize: 28, stickerButtonOffsetX: 0, stickerButtonOffsetY: 0, replyButtonSize: 28, replyButtonOffsetX: 0, replyButtonOffsetY: 0 }
        };
    }
    ...
    function setVars(screen, stateOverride) { const currentState = stateOverride || styleState; screen.style.setProperty('--studio-topbar-height', clamp(currentState.topbar.height, 28, 88) + 'px'); screen.style.setProperty('--studio-topbar-blur', clamp(currentState.topbar.blur, 0, 50) + 'px'); screen.style.setProperty('--studio-bottombar-blur', clamp(currentState.bottombar.blur, 0, 50) + 'px'); screen.style.setProperty('--studio-bottom-bar-height', clamp(currentState.bottombar.height, 60, 160) + 'px'); screen.style.setProperty('--studio-bottom-input-height', clamp(currentState.bottombar.inputHeight, 24, 72) + 'px'); screen.style.setProperty('--studio-bottom-input-radius', clamp(currentState.bottombar.inputRadius, 0, 40) + 'px'); screen.style.setProperty('--studio-bubble-gap', clamp(currentState.layout.bubbleGap, 0, 40) + 'px'); }
    ...
    function normalizeTimestampMode(value) { const normalized = String(value || 'outside').toLowerCase(); return normalized === 'inside' || normalized === 'hidden' ? normalized : 'outside'; }
    ...
    function topbarRules(state) { return [ '#studio-chat-screen .chat-header{height:calc(max(47px, env(safe-area-inset-top, 0px)) + var(--studio-topbar-height)) !important; background-image:' + bgStack(state.bgColor, state.bgOpacity, state.backgroundImage) + ' !important; background-color:transparent !important; -webkit-backdrop-filter:blur(var(--studio-topbar-blur)) !important; backdrop-filter:blur(var(--studio-topbar-blur)) !important; border-bottom:' + borderValue(Number(state.dividerWidth) > 0, state.dividerWidth, state.dividerColor, state.dividerOpacity) + ' !important; box-shadow:' + (state.shadowEnabled ? ('0 1px ' + clamp(state.shadowBlur, 0, 40) + 'px ' + rgba(state.shadowColor, state.shadowOpacity)) : 'none') + ' !important;}', '#studio-chat-screen .chat-header-main{top:calc(max(47px, env(safe-area-inset-top, 0px)) + ' + (clamp(state.height, 28, 88) / 2) + 'px) !important;}', '#studio-chat-screen #studio-chat-messages.chat-body{padding-top:calc(max(47px, env(safe-area-inset-top, 0px)) + ' + (clamp(state.height, 28, 88) + 12) + 'px) !important;}', '#studio-chat-screen #studio-chat-title{color:' + state.textColor + ' !important;}', '#studio-chat-screen .chat-topbar-status{color:' + rgba(state.textColor, 55) + ' !important;}', '#studio-chat-screen .chat-header .back-btn, #studio-chat-screen .chat-header .wechat-icon-btn, #studio-chat-screen .chat-header .multi-select-cancel, #studio-chat-screen .chat-topbar-avatar{color:' + state.iconColor + ' !important;}', '#studio-chat-screen .chat-topbar-avatar{background:' + rgba(state.iconColor, 8) + ' !important;}' ].join('\n'); }
    function topbarDetailRules(state) { return [ '#studio-chat-screen #studio-chat-title{font-size:' + clamp(state.titleFontSize, 10, 30) + 'px !important; position:relative !important; left:' + clamp(state.titleOffsetX, -180, 180) + 'px !important; top:' + clamp(state.titleOffsetY, -120, 120) + 'px !important;}', '#studio-chat-screen .chat-topbar-status{font-size:' + clamp(state.statusFontSize, 8, 24) + 'px !important; position:relative !important; left:' + clamp(state.statusOffsetX, -180, 180) + 'px !important; top:' + clamp(state.statusOffsetY, -120, 120) + 'px !important;}', '#studio-chat-screen #close-studio-app{width:' + clamp(state.backButtonSize, 18, 72) + 'px !important; height:' + clamp(state.backButtonSize, 18, 72) + 'px !important; min-width:' + clamp(state.backButtonSize, 18, 72) + 'px !important; padding:0 !important; justify-content:center !important; transform:translate(' + clamp(state.backButtonOffsetX, -180, 180) + 'px,' + clamp(state.backButtonOffsetY, -120, 120) + 'px) !important;}', '#studio-chat-screen #close-studio-app i{font-size:' + clamp(Math.round(state.backButtonSize * 0.62), 12, 42) + 'px !important;}', '#studio-chat-screen #studio-chat-topbar-avatar-main, #studio-chat-screen #studio-chat-topbar-avatar-right{width:' + clamp(state.avatarButtonSize, 20, 88) + 'px !important; height:' + clamp(state.avatarButtonSize, 20, 88) + 'px !important; min-width:' + clamp(state.avatarButtonSize, 20, 88) + 'px !important; flex-basis:' + clamp(state.avatarButtonSize, 20, 88) + 'px !important; transform:translate(' + clamp(state.avatarButtonOffsetX, -180, 180) + 'px,' + clamp(state.avatarButtonOffsetY, -120, 120) + 'px) !important;}', '#studio-chat-screen #studio-chat-topbar-avatar-main i, #studio-chat-screen #studio-chat-topbar-avatar-right i{font-size:' + clamp(Math.round(state.avatarButtonSize * 0.52), 12, 42) + 'px !important;}', '#studio-chat-screen #studio-chat-settings-btn{width:' + clamp(state.settingsButtonSize, 18, 72) + 'px !important; height:' + clamp(state.settingsButtonSize, 18, 72) + 'px !important; min-width:' + clamp(state.settingsButtonSize, 18, 72) + 'px !important; padding:0 !important; transform:translate(' + clamp(state.settingsButtonOffsetX, -180, 180) + 'px,' + clamp(state.settingsButtonOffsetY, -120, 120) + 'px) !important;}', '#studio-chat-screen #studio-chat-settings-btn i{font-size:' + clamp(Math.round(state.settingsButtonSize * 0.62), 12, 42) + 'px !important;}' ].join('\n'); }
    function bottomRules(state) { return [ '#studio-chat-screen #studio-chat-messages.chat-body{padding-bottom:calc(var(--studio-bottom-bar-height) + 12px) !important;}', '#studio-chat-screen .chat-input-area{min-height:var(--studio-bottom-bar-height) !important; background-image:' + bgStack(state.barBgColor, state.barOpacity, state.backgroundImage) + ' !important; background-color:transparent !important; -webkit-backdrop-filter:blur(var(--studio-bottombar-blur)) !important; backdrop-filter:blur(var(--studio-bottombar-blur)) !important; border-top:' + borderValue(Number(state.dividerWidth) > 0, state.dividerWidth, state.dividerColor, state.dividerOpacity) + ' !important;}', '#studio-chat-screen .chat-input-shell{background:' + state.inputBgColor + ' !important; border:' + clamp(state.inputBorderWidth, 0, 6) + 'px solid ' + state.inputBorderColor + ' !important; border-radius:var(--studio-bottom-input-radius) !important; height:var(--studio-bottom-input-height) !important;}', '#studio-chat-screen #studio-chat-input::placeholder{color:' + rgba(state.iconColor, 60) + ' !important;}', '#studio-chat-screen .chat-icon-btn, #studio-chat-screen .chat-input-inline-btn, #studio-chat-screen #studio-trigger-ai-reply-btn, #studio-chat-screen #studio-sticker-btn{color:' + state.iconColor + ' !important;}', '#studio-chat-screen #studio-chat-more-btn{background:' + rgba(state.iconColor, 10) + ' !important;}' ].join('\n'); }
    function bottomDetailRules(state) { return [ '#studio-chat-screen #studio-chat-more-btn{width:' + clamp(state.menuButtonSize, 20, 88) + 'px !important; height:' + clamp(state.menuButtonSize, 20, 88) + 'px !important; min-width:' + clamp(state.menuButtonSize, 20, 88) + 'px !important; flex-basis:' + clamp(state.menuButtonSize, 20, 88) + 'px !important; font-size:' + clamp(Math.round(state.menuButtonSize * 0.59), 12, 42) + 'px !important; transform:translate(' + clamp(state.menuButtonOffsetX, -180, 180) + 'px,' + clamp(state.menuButtonOffsetY, -120, 120) + 'px) !important;}', '#studio-chat-screen #studio-sticker-btn{width:' + clamp(state.stickerButtonSize, 20, 88) + 'px !important; height:' + clamp(state.stickerButtonSize, 20, 88) + 'px !important; min-width:' + clamp(state.stickerButtonSize, 20, 88) + 'px !important; flex-basis:' + clamp(state.stickerButtonSize, 20, 88) + 'px !important; transform:translate(' + clamp(state.stickerButtonOffsetX, -180, 180) + 'px,' + clamp(state.stickerButtonOffsetY, -120, 120) + 'px) !important;}', '#studio-chat-screen #studio-sticker-btn i{font-size:' + clamp(Math.round(state.stickerButtonSize * 0.79), 12, 48) + 'px !important;}', '#studio-chat-screen #studio-trigger-ai-reply-btn{width:' + clamp(state.replyButtonSize, 20, 88) + 'px !important; height:' + clamp(state.replyButtonSize, 20, 88) + 'px !important; min-width:' + clamp(state.replyButtonSize, 20, 88) + 'px !important; flex-basis:' + clamp(state.replyButtonSize, 20, 88) + 'px !important; font-size:' + clamp(Math.round(state.replyButtonSize * 0.86), 12, 48) + 'px !important; transform:translate(' + clamp(state.replyButtonOffsetX, -180, 180) + 'px,' + clamp(state.replyButtonOffsetY, -120, 120) + 'px) !important;}' ].join('\n'); }
    ...
    function buildCssForState(currentState) { return [ bubbleRules('.chat-message.other[data-preview-group="ordinary"]', '.message-content', currentState.ordinary.other, false), bubbleRules('.chat-message.user[data-preview-group="ordinary"]', '.message-content', currentState.ordinary.user, false), bubbleRules('.chat-message.other[data-preview-group="voice"]', '.message-content.voice-msg', currentState.voice.other, true), bubbleRules('.chat-message.user[data-preview-group="voice"]', '.message-content.voice-msg', currentState.voice.user, true), timestampRules('.chat-message.other[data-preview-group="ordinary"]', '.message-content', currentState.ordinary.other), timestampRules('.chat-message.user[data-preview-group="ordinary"]', '.message-content', currentState.ordinary.user), timestampRules('.chat-message.other[data-preview-group="voice"]', '.message-content.voice-msg', currentState.voice.other), timestampRules('.chat-message.user[data-preview-group="voice"]', '.message-content.voice-msg', currentState.voice.user), quoteRules('other', currentState.quoted.other), quoteRules('user', currentState.quoted.user), avatarRules(currentState), topbarRules(currentState.topbar), topbarDetailRules(currentState.topbar), bottomRules(currentState.bottombar), bottomDetailRules(currentState.bottombar) ].join('\n'); }
    ...
    function ensureTopbarAdvancedControls(root) {
        ...
        wrapper.innerHTML = '' +
            '<div class="control-group">' +
                '<div class="control-label" style="margin-bottom: 0;"><span style="font-weight: 600; color: var(--text-main);"><i class="ri-text"></i> 顶栏状态文字</span><label class="switch"><input type="checkbox" id="studio-topbar-status-visible-toggle"><span class="slider"></span></label></div>' +
                '<div class="collapsible-panel" id="studio-topbar-status-text-panel" style="border-top: none; padding-top: 8px; margin-top: 0;">' +
                    '<div class="control-label"><span>状态文字内容</span></div>' +
                    '<div class="url-input-wrapper"><i class="ri-edit-2-line"></i><input type="text" id="studio-topbar-status-text-input" placeholder="输入状态文字..."></div>' +
                '</div>' +
            '</div>' +
            '<div class="control-group">' +
                '<div class="control-label" style="color: var(--text-main); font-weight: 600;"><span><i class="ri-font-size-2"></i> 名字与状态</span></div>' +
                '<div class="control-label" style="margin-top: 8px;"><span>名字大小</span><span class="value-display" id="studio-topbar-title-size-display">17px</span></div><input type="range" id="studio-topbar-title-size" min="10" max="30" value="17" style="margin-bottom: 8px;">' +
                '<div class="control-label"><span>名字横向位置</span><span class="value-display" id="studio-topbar-title-offset-x-display">0px</span></div><input type="range" id="studio-topbar-title-offset-x" min="-180" max="180" value="0" style="margin-bottom: 8px;">' +
                '<div class="control-label"><span>名字纵向位置</span><span class="value-display" id="studio-topbar-title-offset-y-display">0px</span></div><input type="range" id="studio-topbar-title-offset-y" min="-80" max="80" value="0" style="margin-bottom: 8px;">' +
                '<div class="control-label"><span>状态大小</span><span class="value-display" id="studio-topbar-status-size-display">11px</span></div><input type="range" id="studio-topbar-status-size" min="8" max="24" value="11" style="margin-bottom: 8px;">' +
                '<div class="control-label"><span>状态横向位置</span><span class="value-display" id="studio-topbar-status-offset-x-display">0px</span></div><input type="range" id="studio-topbar-status-offset-x" min="-180" max="180" value="0" style="margin-bottom: 8px;">' +
                '<div class="control-label"><span>状态纵向位置</span><span class="value-display" id="studio-topbar-status-offset-y-display">0px</span></div><input type="range" id="studio-topbar-status-offset-y" min="-80" max="80" value="0">' +
            '</div>' +
            '<div class="control-group">' +
                '<div class="control-label" style="color: var(--text-main); font-weight: 600;"><span><i class="ri-arrow-left-line"></i> 返回按钮</span></div>' +
                '<div class="control-label" style="margin-top: 8px;"><span>按钮大小</span><span class="value-display" id="studio-topbar-back-size-display">28px</span></div><input type="range" id="studio-topbar-back-size" min="18" max="72" value="28" style="margin-bottom: 8px;">' +
                '<div class="control-label"><span>横向位置</span><span class="value-display" id="studio-topbar-back-offset-x-display">0px</span></div><input type="range" id="studio-topbar-back-offset-x" min="-180" max="180" value="0" style="margin-bottom: 8px;">' +
                '<div class="control-label"><span>纵向位置</span><span class="value-display" id="studio-topbar-back-offset-y-display">0px</span></div><input type="range" id="studio-topbar-back-offset-y" min="-80" max="80" value="0">' +
            '</div>' +
            '<div class="control-group">' +
                '<div class="control-label" style="color: var(--text-main); font-weight: 600;"><span><i class="ri-user-3-line"></i> 头像按钮</span></div>' +
                '<div class="control-label" style="margin-top: 8px;"><span>按钮大小</span><span class="value-display" id="studio-topbar-avatar-size-display">34px</span></div><input type="range" id="studio-topbar-avatar-size" min="20" max="88" value="34" style="margin-bottom: 8px;">' +
                '<div class="control-label"><span>横向位置</span><span class="value-display" id="studio-topbar-avatar-offset-x-display">0px</span></div><input type="range" id="studio-topbar-avatar-offset-x" min="-180" max="180" value="0" style="margin-bottom: 8px;">' +
                '<div class="control-label"><span>纵向位置</span><span class="value-display" id="studio-topbar-avatar-offset-y-display">0px</span></div><input type="range" id="studio-topbar-avatar-offset-y" min="-80" max="80" value="0">' +
            '</div>' +
            '<div class="control-group">' +
                '<div class="control-label" style="color: var(--text-main); font-weight: 600;"><span><i class="ri-settings-3-line"></i> 设置按钮</span></div>' +
                '<div class="control-label" style="margin-top: 8px;"><span>按钮大小</span><span class="value-display" id="studio-topbar-settings-size-display">28px</span></div><input type="range" id="studio-topbar-settings-size" min="18" max="72" value="28" style="margin-bottom: 8px;">' +
                '<div class="control-label"><span>横向位置</span><span class="value-display" id="studio-topbar-settings-offset-x-display">0px</span></div><input type="range" id="studio-topbar-settings-offset-x" min="-180" max="180" value="0" style="margin-bottom: 8px;">' +
                '<div class="control-label"><span>纵向位置</span><span class="value-display" id="studio-topbar-settings-offset-y-display">0px</span></div><input type="range" id="studio-topbar-settings-offset-y" min="-80" max="80" value="0">' +
            '</div>';
    ...
    function ensureBottomAdvancedControls(root) {
        ...
        wrapper.innerHTML = '' +
            '<div class="control-group">' +
                '<div class="control-label" style="color: var(--text-main); font-weight: 600;"><span><i class="ri-layout-bottom-line"></i> 底栏高度</span></div>' +
                '<div class="control-label" style="margin-top: 8px;"><span>底栏高度</span><span class="value-display" id="studio-bottombar-height-display">83px</span></div><input type="range" id="studio-bottombar-height" min="60" max="160" value="83">' +
            '</div>' +
            '<div class="control-group">' +
                '<div class="control-label" style="color: var(--text-main); font-weight: 600;"><span><i class="ri-add-circle-line"></i> 加号按钮</span></div>' +
                '<div class="control-label" style="margin-top: 8px;"><span>按钮大小</span><span class="value-display" id="studio-bottombar-menu-size-display">34px</span></div><input type="range" id="studio-bottombar-menu-size" min="20" max="88" value="34" style="margin-bottom: 8px;">' +
                '<div class="control-label"><span>横向位置</span><span class="value-display" id="studio-bottombar-menu-offset-x-display">0px</span></div><input type="range" id="studio-bottombar-menu-offset-x" min="-180" max="180" value="0" style="margin-bottom: 8px;">' +
                '<div class="control-label"><span>纵向位置</span><span class="value-display" id="studio-bottombar-menu-offset-y-display">0px</span></div><input type="range" id="studio-bottombar-menu-offset-y" min="-120" max="120" value="0">' +
            '</div>' +
            '<div class="control-group">' +
                '<div class="control-label" style="color: var(--text-main); font-weight: 600;"><span><i class="ri-sticker-line"></i> 贴纸按钮</span></div>' +
                '<div class="control-label" style="margin-top: 8px;"><span>按钮大小</span><span class="value-display" id="studio-bottombar-sticker-size-display">28px</span></div><input type="range" id="studio-bottombar-sticker-size" min="20" max="88" value="28" style="margin-bottom: 8px;">' +
                '<div class="control-label"><span>横向位置</span><span class="value-display" id="studio-bottombar-sticker-offset-x-display">0px</span></div><input type="range" id="studio-bottombar-sticker-offset-x" min="-180" max="180" value="0" style="margin-bottom: 8px;">' +
                '<div class="control-label"><span>纵向位置</span><span class="value-display" id="studio-bottombar-sticker-offset-y-display">0px</span></div><input type="range" id="studio-bottombar-sticker-offset-y" min="-120" max="120" value="0">' +
            '</div>' +
            '<div class="control-group">' +
                '<div class="control-label" style="color: var(--text-main); font-weight: 600;"><span><i class="ri-mic-line"></i> 语音按钮</span></div>' +
                '<div class="control-label" style="margin-top: 8px;"><span>按钮大小</span><span class="value-display" id="studio-bottombar-reply-size-display">28px</span></div><input type="range" id="studio-bottombar-reply-size" min="20" max="88" value="28" style="margin-bottom: 8px;">' +
                '<div class="control-label"><span>横向位置</span><span class="value-display" id="studio-bottombar-reply-offset-x-display">0px</span></div><input type="range" id="studio-bottombar-reply-offset-x" min="-180" max="180" value="0" style="margin-bottom: 8px;">' +
                '<div class="control-label"><span>纵向位置</span><span class="value-display" id="studio-bottombar-reply-offset-y-display">0px</span></div><input type="range" id="studio-bottombar-reply-offset-y" min="-120" max="120" value="0">' +
            '</div>';
        container.appendChild(wrapper);
    }
    ...
    function syncBottom(root, bindings) { loadBindings(bindings, styleState.bottombar); syncBottomAdvancedControls(root); }
