    ...
    function runStudioToolbar(root) {
        // ================= 全局状态 =================
                ...
                function ensureStudioPresetActionButtons() {
                    ...
                    if (applyToWechatBtn) applyToWechatBtn.dataset.studioHideWhenCollapsed = 'true';
                    ...
                    const makeHeaderButton = (id, label, icon) => {
                        ...
                        headerActions.insertBefore(button, applyToWechatBtn || collapseBtn);
                        return button;
                    };

                    const importPresetBtn = makeHeaderButton('importStudioPresetBtn', '导入外观', 'ri-upload-2-line');
                    const exportPresetBtn = makeHeaderButton('exportStudioPresetBtn', '导出外观', 'ri-download-2-line');
                ...
                if (applyToWechatBtn) {
                    applyToWechatBtn.addEventListener('click', (e) => {
                        if (!isClick) return;
                        e.preventDefault();
                        e.stopPropagation();
                        if (typeof window.applyStudioStylesToWechat === 'function') {
                            window.applyStudioStylesToWechat();
                            applyToWechatBtn.style.color = 'var(--accent)';
                            applyToWechatBtn.style.background = 'rgba(0, 122, 255, 0.14)';
                        }
                    });
                }
